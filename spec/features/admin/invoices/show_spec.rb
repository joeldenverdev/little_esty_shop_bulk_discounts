...
describe 'Admin Invoices Index Page' do
  ...
  describe 'admin show page total revenue information' do
    before :each do
      ...
      @ii_2 = InvoiceItem.create!(invoice_id: @invoice_2.id, item_id: @item_1.id, quantity: 16, unit_price: 10, status: 2)
    ...
    it 'should display the total revenue without discounts' do
      visit admin_invoice_path(@invoice_1.id)
      expect(page).to have_content("Total Revenue: $172.0")
    end

    it 'should display the total discount' do
        visit admin_invoice_path(@invoice_1.id)
      expect(page).to have_content("Total Discount: $17.2")
    end

    it ' should display the discounts applied' do
        visit admin_invoice_path(@invoice_1.id)
      expect(page).to have_content("Discount Applied: 10%")
    end

    it 'should display the total revenue after discounts' do
        visit admin_invoice_path(@invoice_1.id)
      expect(page).to have_content("Total After Discount: $154.8")
    end

    it 'should display the total revenue without discounts' do
      visit admin_invoice_path(@invoice_3.id)
      expect(page).to have_content("Total Revenue: $16.0")
    end

    it 'should display the total discount' do
        visit admin_invoice_path(@invoice_3.id)
      expect(page).to have_content("Total Discount: $0.0")
    end
    ...
    it 'should display the total revenue after discounts' do
        visit admin_invoice_path(@invoice_3.id)
      expect(page).to have_content("Total After Discount: $16.0")
    end
  end
end
